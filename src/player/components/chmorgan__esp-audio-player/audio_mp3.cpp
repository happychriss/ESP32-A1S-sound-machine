                  ...
                  (magic[2] == 0x33)) /* 'ID3' */
        {
            ...
            if (sizeof(mp3_id3_header_v2_t) == fread(&tag, 1, sizeof(mp3_id3_header_v2_t), fp)) {
                if (memcmp("ID3", (const void *) &tag, sizeof(tag.header)) == 0) {
                    ...
                    /* Seek past the entire ID3 tag (syncsafe size, 10-byte header not included).
                     * Without this, the decoder scans through potentially hundreds of KB of
                     * cover-art JPEG binary looking for the first MP3 sync word, starving
                     * the I2S DMA and causing audio glitches at the start of every song. */
                    uint32_t id3_size = (((uint8_t)tag.size[0] & 0x7f) << 21) |
                                        (((uint8_t)tag.size[1] & 0x7f) << 14) |
                                        (((uint8_t)tag.size[2] & 0x7f) <<  7) |
                                         ((uint8_t)tag.size[3] & 0x7f);
                    fseek(fp, (long)(10 + id3_size), SEEK_SET);
                    return is_mp3_file;   /* file pointer now at first MP3 frame */
                }
            }
        }
    }
...
0);
        ...
        if(mp3_dec_err == ERR_MP3_NONE) {
            ...
            /* Guard: nChans==0 means the decoder processed garbage data (e.g. ID3
             * cover-art JPEG that looks like a valid frame header). Skip this frame. */
